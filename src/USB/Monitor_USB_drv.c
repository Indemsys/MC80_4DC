...
static uint32_t _Create_receiving_task(T_usb_serial_drv_cbl *dcbl_ptr, char *task_name, char *evg_name)
{
  ...
  if (res == TX_SUCCESS)
  {
    EAPPLOG("USB receiver task created.");
    return RES_OK;
  }
  else
  {
    tx_event_flags_delete(dcbl_ptr->event_flags_ptr);
    EAPPLOG("USB receiver task creating error %04X.", res);
    return RES_ERROR;
  }
}
...
static void Init_monitor_usb_driver(T_vt100_man_msg *msg_ptr)
{
  ...
  if (module_inited == 0)
  {
    // Очищаем область вручную поскольку она не очищается стартапным кодом, поскольку находится в секции .usb_cbls
  ...
  for (uint32_t i = 0; i < USB_VT100_DRV_INSTANCES_MAX_NUM; i++)
  {
    if (usbd_cbl[i].taken == 0)
    {
      ...
      if (res == RES_OK)
      {
        ...
        EAPPLOG("USB VCOM VT100 task %d created", usbd_cbl[instance_indx].vt100_task_instance_index);
      }
      else
      {
        goto err_exit;
  ...
  if (instance_indx == -1)
  {
    EAPPLOG("USB VCOM VT100 task creating error");
  }
  return;
...
static void DeInit_monitor_usb_driver(T_vt100_man_msg *msg_ptr)
{
  ...
  for (uint32_t i = 0; i < USB_VT100_DRV_INSTANCES_MAX_NUM; i++)
  {
    if (usbd_cbl[i].cdc == (UX_SLAVE_CLASS_CDC_ACM *) cdc_instance)
    {
      ...
      usbd_cbl[instance_indx].taken = 0;
      usbd_cbl[instance_indx].cdc = NULL;
      EAPPLOG("USB VCOM VT100 task %d deleted", usbd_cbl[instance_indx].vt100_task_instance_index);
    }
  }
  if (instance_indx == -1)
  {
    EAPPLOG("USB VCOM VT100 task deleting error");
  }
}
